...
void main() {
  ...
  testWidgets(
    'OnboardingScreen renders step 1 preferences and advances steps',
    (WidgetTester tester) async {
      final firestore = FakeFirebaseFirestore();
      final auth = MockFirebaseAuth(
        signedIn: true,
        mockUser: MockUser(uid: 'onboarding-user'),
      );
        ...
        ProviderScope(
          overrides: [
            ...
            localeProvider.overrideWith(
              (ref) =>
                  LocaleNotifier(AppSettingsStore())
                    ..setLocale(const Locale('tr')),
            ),
            onboardingCompletedProvider.overrideWith(
              (ref) =>
                  OnboardingCompletedNotifier(AppSettingsStore())
                    ..setCompleted(false),
            ),
      ...
      final favorite = await firestore
          .collection('users')
          .doc('onboarding-user')
          .collection('movie_settings')
          .doc('101_false')
          .get();
      expect(favorite.data()?['isFavorite'], isTrue);
