...
void main() {
  ...
  testWidgets(
    'hovering a poster in the filmstrip scales it up and un-scales on exit',
    (tester) async {
        ...
        ProviderScope(
          overrides: [
            firebaseAuthProvider.overrideWithValue(
              MockFirebaseAuth(
                signedIn: true,
                mockUser: MockUser(uid: 'me', email: 'me@test.com'),
              ),
            ),
            communityFeedProvider.overrideWith(
              (ref) => Stream.value([_diarySnapshotPost()]),
            ),
            followedUserIdsProvider.overrideWith(
  ...
  testWidgets('poster previews use the web-safe image proxy', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          firebaseAuthProvider.overrideWithValue(
    ...
    final provider = poster.image as NetworkImage;
    expect(provider.url, startsWith(ApiConstants.imagePathW185));
    expect(provider.url, isNot(startsWith('https://image.tmdb.org')));
  });

  testWidgets(
    'pressing a poster on touch grows it too, and lifting still navigates (tap not swallowed)',
    (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
  ...
  testWidgets(
    'shows a "+N" tile for entries beyond the ones with posters shown',
    (tester) async {
