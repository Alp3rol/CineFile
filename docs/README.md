# CineFile dokümantasyonu

Bu klasör, uygulamanın ürün planlarını ve geliştirici belgelerini konu başlıklarına göre toplar.

## Planlama

- [Aktif ürün ve teknik yol haritası](planning/ROADMAP.md)
- [Sürüm geçmişi ve eski yol haritası](planning/HISTORY.md)

## Mimari

- [Mimari ve ürün denetimi](architecture/AUDIT.md)
- [Hata işleme politikası](architecture/ERROR_HANDLING.md)

## Firebase ve veri güvenliği

- [Firebase App Check geçiş planı](firebase/APP_CHECK_ROLLOUT.md)
- [Firestore veri koruma planı](firebase/FIRESTORE_DISASTER_RECOVERY.md)
- [Firestore yazma izin matrisi](firebase/FIRESTORE_RULES_MATRIX.md)

## Operasyon

- [Sentry sürüm bilgileri](operations/SENTRY_RELEASES.md)
- [Sentetik sağlık kontrolü](operations/SYNTHETIC_HEALTH.md)
- [Web yayın geri alma (rollback) prosedürü](operations/WEB_ROLLBACK_PROCEDURE.md)

## Kalite

- [Test kapsamı tabanı](quality/COVERAGE_BASELINE.md)
- [Test performansı raporu](quality/TEST_PERFORMANCE_2026-08-05.md)
