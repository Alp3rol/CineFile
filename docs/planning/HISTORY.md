...
> uygulama sırası için [`ROADMAP.md`](ROADMAP.md) kullanılmalıdır.
...
#### **✅ v1.7.2: Bölüm Rehberi İnce Ayarları — Sıradaki Bölüm, Toplu İzleme, Sezon Afişi, Bölüm Bildirimi, İçgörü Entegrasyonu**
*   **Hedef**: v1.7.1'de temellenen Bölüm Rehberi'ni küçük/orta efor beş geliştirmeyle zenginleştirmek: bir sonraki bölümü görsel olarak öne çıkarmak, sezonu tek tıkla tamamlamak, sezon afişlerini göstermek, sıradaki yayınlanmamış bölüm için bildirim planlamak, ve sadece ilerleme sayacıyla takip edilen (günlüğe hiç girilmemiş) izleme etkinliğini İçgörüler/heatmap'e dedup'lı şekilde katmak.
*   **İşler**:
    *   **✅ Paylaşılan bölüm matematiği**: Sezon sıralama + overall-index hesaplama mantığı `tv_episodes_section.dart`'tan yeni `lib/core/utils/tv_episode_math.dart`'a çıkarıldı (`sortedRegularSeasons`, `calculateOverallEpisodeNumber`, ve yeni ters-eşleme `mapOverallIndexToSeasonEpisode`), hem UI hem `NotificationService` tarafından paylaşılıyor.
    *   **✅ Sıradaki Bölüm Rozeti**: `TvEpisodeListItem`'a `isNextUp` parametresi eklendi; bir sonraki izlenmemiş bölüm "▶ SIRADAKİ" rozeti ve amber kenarlıkla vurgulanıyor. (Otomatik scroll yerine sadece görsel rozet tercih edildi — üst ekranın `ScrollController`'ında v1.4.3'te yaşanan scroll-jump hatasının riskli alanına tekrar dokunmamak için.)
    *   **✅ "Bu Sezonu İzledim" Toplu Buton**: Seçili sezonun son bölümüne kadar tek tıkla işaretleme; mevcut `_toggleEpisodeWatched`'ın bitişik-olmayan-hedef onay diyaloğu ve günlük-ekleme prompt'u aynen devreye giriyor. Sezon zaten tamamlanmışsa buton gizleniyor.
    *   **✅ Sezon Afişi**: `TvSeasonChipRow`'daki sezon çiplerine TMDb'nin `poster_path` alanından 24x24 küçük afiş eklendi (`seed` ile placeholder fallback).
    *   **✅ Sıradaki Bölüm İçin Yayın Tarihi Bildirimi**: `NotificationService`'e `scheduleNextEpisodeReminder`/`cancelNextEpisodeReminder` eklendi — ayrı ID namespace'i (`'ep_$tmdbId'.hashCode`) ve payload öneki (`episode:`) ile mevcut çıkış-hatırlatıcısıyla çakışmıyor. `syncNotifications()` artık watchlist'ten bağımsız olarak "Aktif İzliyorum" durumundaki her dizi için TMDb'den sıradaki bölümün yayın tarihini çekip (sadece o tek bölüm için) bildirim planlıyor — sezondaki tüm yayınlanmamış bölümler için değil, bildirim yorgunluğunu önlemek amacıyla.
    *   **✅ İçgörüler/Heatmap Dedup Entegrasyonu**: `UserMovieSettings`'e yıkıcı olmayan yeni bir alan eklendi (`lastEpisodeProgressAt`, schema v11, `m.addColumn`) — genel `updatedAt`'ten ayrı tutuluyor çünkü o alan favori/puan/not gibi alakasız yazımlarda da değişiyor. `insightsProvider` artık `allMovieSettingsProvider`'ı da izleyip, günlüğe hiç kayıt girilmemiş ama ilerleme sayacıyla takip edilen günleri streak/heatmap'e katıyor — aynı gün için zaten gerçek bir `WatchRecords` kaydı varsa çift saymıyor.
    *   Yeni testler: `test/tv_episode_math_test.dart` (10 birim testi), `tv_episodes_section_test.dart`'a 4 yeni widget testi (rozet, toplu buton × 2), `insights_provider_test.dart`'a 2 yeni test (dedup'lı/dedup'sız senaryolar). Toplam paket 71 → 90 teste çıktı.
    *   `dart analyze lib` ve `flutter test` (90/90) temiz.
...
Uygulama planlanan tüm MVP aşamalarını ve büyük sosyal özellikleri (Faz 3, 4, 5) başarıyla tamamlamıştır. v1.4.x serisi arayüz/marka cilası ile kapandı. v1.6.0 (Keşfet'te Trend/Popüler/En Çok Oy Alan filtreleri), v1.7.0 (oyuncu kadrosunu tıklanabilir kılan Oyuncu Profili), v1.7.1 (dizilerde TMDb sezon/bölüm verisiyle beslenen Gelişmiş Bölüm Rehberi + günlüğe eklenmemiş dizilerde bölüm işaretleme diyaloğu) ve v1.7.2 (Bölüm Rehberi ince ayarları: sıradaki bölüm rozeti, toplu sezon işaretleme, sezon afişi, bölüm bildirimi, İçgörüler entegrasyonu) tamamlandı. Bildirim/moderasyon katmanı ve "CineFile Wrapped" yıl sonu özeti (eski v1.6.1/v1.6.2) tarih belirtilmeden `vx.x.x` olarak zamanlanmamış/backlog kalemleri haline getirildi.
